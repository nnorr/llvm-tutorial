...
파서가 알려주지 못하는 실패가 하나 있어서, 루프가 끝난 뒤 렉서에게 따로
물어봅니다.

```cpp
// 거부된 토큰도 값을 돌려주므로, 렉서가 이미 불평한 입력에도 파싱이
// 성공할 수 있습니다. .o를 쓰기 전에 확인합니다.
if (Lex.hadError())
  Failed = true;
```

잘못된 숫자 리터럴은 진단을 찍은 뒤 `tok_number`와 `0.0`을 돌려줍니다
([01-lexer](01-lexer.md) 4절). 파서 입장에서는 정상 숫자와 구별되지 않으므로,
이 확인이 없으면 컴파일러가 스스로 거부한 프로그램의 `.o`를 만들어 냅니다.
`test/driver/bad-number.ks`가 이 경로를 지킵니다.
